    ...
    public partial class EditTaskWindow : Window
    {
        ...
        public EditTaskWindow(TaskItem task)
        {
            ...
            _originalTask = new TaskItem
            {
                ...
                CategoryId = task.CategoryId, ResourceId = task.ResourceId, IsCompleted = task.IsCompleted
            };
        ...
        private async void PopulateForm()
        {
            ...
            var resources = (await resourceRepo.GetAllAsync()).ToList();
            resources.Insert(0, new ResourceItem { Id = 0, Title = "— Kaynak Yok (Temizle) —" });
            CmbResource.ItemsSource = resources;
            CmbResource.SelectedValue = _task.ResourceId ?? 0;

            // Mevcut zincirleri yükle
            var allTasks = await _repository.GetAllTasksAsync();
            var chains = allTasks.Where(t => t.TaskType == TaskType.Zincirleme && !string.IsNullOrEmpty(t.ChainId))
                                 .Select(t => t.ChainId!)
                                 .Distinct()
                                 .ToList();
        ...
        private async void BtnSave_Click(object sender, RoutedEventArgs e)
        {
            ...
            int? selectedResourceId = CmbResource.SelectedValue as int?;
            if (selectedResourceId == 0) selectedResourceId = null;
            ...
            _task.ResourceId = selectedResourceId;
